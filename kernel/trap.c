...
void time_save(struct proc *p);

//
// handle an interrupt, exception, or system call from user space.
// called from trampoline.S
//
// 首先需要知道的是进入当前函数已经进行了上下文的保存在trampoline.S汇编代码中,并将相关的值保存到trapframe中
void usertrap(void) {
  int which_dev = 0;
  ...
  if (which_dev == 2) {
    ...
    if (sys_uptime() >= p->interval + p->last_call && p->interval != 0 &&
      ...
      p->save.over = 1;
      // 当前的程序最终会执行的sys_sigreturn寄存器,当前需要保存相关的寄存器到proc中,需要保存用户常用的寄存器,以及pc的值
    }
    yield();
  }
  usertrapret();
